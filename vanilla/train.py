...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = WordLevelTrainer(special_tokens=["[UNK]", "[SOS]", "[PAD]", "[EOS]"], min_frequency=2, show_progress=True)
...
def train_model(config):
    ...
    Path(config['model_folder']).mkdir(parents=True, exist_ok=True) # make sure the weights folder is created
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # the first argument of the following function transforms dimensions as follows
